 ...
 */
@WebServlet("/noticeUpdateReply.bo")
public class NoticeUpdateReplyController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NoticeUpdateReplyController() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int result = replyService.noticeUpdateReply(reply);

     // 결과에 따라 적절한 응답 생성 (성공 또는 실패 페이지로 리다이렉션)
        if (result > 0) {
            response.sendRedirect("noticedetail.bo?bno="+bno); // bno를 파라미터로 전달하도록 수정
        } else {
            // 실패 처리
