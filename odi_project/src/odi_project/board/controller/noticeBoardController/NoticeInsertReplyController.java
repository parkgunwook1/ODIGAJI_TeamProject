 ...
 */
@WebServlet("/noticeInsertReply.bo")
public class NoticeInsertReplyController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public NoticeInsertReplyController() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
	    ...
	    try {
	        ...
	        if (boardNoParam != null) {
	            ...
	            int result = boardService.noticeInsertReply(reply);

	            if (result > 0) {
	                // 댓글 작성 성공 시
	                response.sendRedirect("noticedetail.bo?bno=" + boardNo);
	            } else {
	                // 댓글 작성 실패 시
	                // 실패 처리를 원하는 방식으로 구현
